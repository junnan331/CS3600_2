...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        st = self.states.get((state,action),0.0)
        return st
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):
          lAction = self.getLegalActions(state)
          if lAction:
              return random.choice(lAction)
        else:
    ...
    def update(self, state, action, nextState, reward):
        ...
        newV = self.computeValueFromQValues(nextState)

        #  Q-learning formula
        update = (1 - self.alpha) * oldQ + self.alpha * (reward + self.discount * newV)
